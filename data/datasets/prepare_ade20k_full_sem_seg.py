...
from models.Mask2former.mask2former.data.datasets.register_ade20k_full import ADE20K_SEM_SEG_FULL_CATEGORIES


def loadAde20K(file):
    """
    加载 ADE20K 数据集的单张图像及其对应的分割标注信息。

    参数:
    file (str): 图像文件的路径，格式为 .jpg

    返回:
    dict: 包含图像文件名、分割标注文件名和类别掩码的字典
        - "img_name": 图像文件的路径
        - "segm_name": 分割标注文件的路径，格式为 .png
        - "class_mask": 表示图像中每个像素所属类别的掩码数组
    """
